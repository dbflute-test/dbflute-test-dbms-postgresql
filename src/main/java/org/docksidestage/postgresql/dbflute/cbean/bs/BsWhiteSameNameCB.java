...
public class BsWhiteSameNameCB extends AbstractConditionBean {
     ...
     * <pre>
     * <span style="color: #0000C0">whiteSameNameBhv</span>.selectEntity(<span style="color: #553000">cb</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     <span style="color: #553000">cb</span>.<span style="color: #CC4747">setupSelect_NextSchemaProduct()</span>; <span style="color: #3F7E5E">// ...().with[nested-relation]()</span>
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">whiteSameName</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">whiteSameName</span>.<span style="color: #CC4747">getNextSchemaProduct()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
    ...
    public NextSchemaProductNss setupSelect_NextSchemaProduct() {
        ...
        doSetupSelect(() -> query().queryNextSchemaProduct());
    ...
    public static class HpSpecification extends HpAbstractSpecification<WhiteSameNameCQ> {
        ...
        public NextSchemaProductCB.HpSpecification specifyNextSchemaProduct() {
            ...
            if (_nextSchemaProduct == null) {
                _nextSchemaProduct = new NextSchemaProductCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryNextSchemaProduct()
                                    , () -> _qyCall.qy().queryNextSchemaProduct())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _nextSchemaProduct.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryNextSchemaProduct()
                      , () -> xsyncQyCall().qy().queryNextSchemaProduct()));
                }
            }
         ...
         */
        public HpSDRFunction<WhiteSameNameRefCB, WhiteSameNameCQ> derivedWhiteSameNameRef() {
            assertDerived("whiteSameNameRefList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<WhiteSameNameRefCB> sq, WhiteSameNameCQ cq, String al, DerivedReferrerOption op)
